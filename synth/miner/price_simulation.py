...
def simulate_price_path_student_t(S0, time_increment, time_length, sigma_hourly, asset = "BTC"):
    ...
    num_steps = int(time_length / time_increment) + 1

    theT = 1 / 365

    if time_length < 24 * 3600:
        theT = time_length / (365 * 24 * 3600)
    
    ST, R = sample_student_t_multistep(S0, sigma_atm, T=theT, nu=nu, n_paths=1, n_steps=num_steps)
